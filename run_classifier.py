...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {'input_ids': batch[0],  # inputFeature里的排序
                        'attention_mask': batch[1],
                        'input_tag_ids': batch[4],
                        'input_def_ids': batch[5],
                        'input_tag_label':batch[6],
                        # 'input_tag_mask':batch[7],
                        'labels': batch[7]}
            inputs['token_type_ids'] = batch[2]
            outputs, tag_loss, tag_acc = model(**inputs)
            loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
            # 中间check evaluate的结果：
            if (step+1) % 200 == 0:
                evaluate(args, model, tokenizer)
                # test_sample(args, model, tokenizer)
            #end
            ...
            if model.train():
                summary_writer.add_scalar('wsd_loss', tag_loss.detach().cpu() * 0.6, (_ * len(train_dataloader)) + step)
                summary_writer.add_scalar('match_loss', (loss-tag_loss).detach().cpu(), (_ * len(train_dataloader)) + step)
                summary_writer.add_scalar('tag_acc', tag_acc, (_ * len(train_dataloader)) + step)
                # summary_writer.add_scalar('total_loss', loss.detach().cpu(), (_ * len(train_dataloader)) + step)
...
# def test_sample(args, model, tokenizer, prefix=""):
...
#         eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, data_type='test')
...
#         preds = None
#         out_label_ids = None
#         pbar = ProgressBar(n_total=len(eval_dataloader), desc="Testing")
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.int)
    all_lens = torch.tensor([f.input_len for f in features], dtype=torch.int8)

    temp_tag_id = [f.input_tag_ids for f in features]
    temp_def_id = [f.input_def_ids for f in features]

    all_tag_ids = torch.tensor(temp_tag_id, dtype=torch.int)
    all_def_ids = torch.tensor(temp_def_id, dtype=torch.int)

    all_tag_label = torch.tensor([f.input_tag_label for f in features], dtype=torch.long)
    # all_tag_mask = torch.tensor([f.input_tag_mask for f in features], dtype=torch.long)
...
def main():
    ...
    parser.add_argument("--num_labels", default=None, type=int, required=True,
                        help="number of labels in this task")
    ...
    tag_vocab_file = os.path.join(args.data_dir, 'tag_vocab.txt')
    # def_file = os.path.join(args.data_dir, 'definition.txt')
    ...
    tokenizer = tokenization_albert.FullTokenizer(vocab_file=args.vocab_file, tag_vocab_file=tag_vocab_file, do_lower_case=args.do_lower_case,
